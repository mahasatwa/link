...
export async function GET(_request: Request, { params }: RouteParams) {
  ...
  const link = await getLinkById(Number(params.id));
...
export async function PUT(request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    const existing = await getLinkById(Number(params.id));
    if (!existing) {
      return NextResponse.json({ error: "Tautan tidak ditemukan" }, { status: 404 });
    }
    ...
    if (slugConflict && slugConflict.id !== Number(params.id)) {
      return NextResponse.json(
    ...
    await updateLink(Number(params.id), {
      slug,
      target_url,
      title,
      category: category || "lainnya",
    });
    const updated = await getLinkById(Number(params.id));
